...
from app.models import (
    ...
    HealthMetric, BuildMetric, TestResultsTag,
    ReportTemplate, ReportSchedule, Report
)
from app.models.database import TestStatus, ReportFormat, ReportStatus, ReportType
...
def generate_build_info(db: Session, project: Project, count: int) -> List[BuildInfo]:
    ...
    for i in range(count):
        ...
        db.add(build)
        db.flush()  # This gets the ID assigned without a full commit
        builds.append(build)

        # Add build metrics AFTER build has an ID
        for metric_name in ["cpu_usage", "memory_usage", "disk_usage", "network_io"]:
            metric = BuildMetric(
                build_id=build.id,  # Now build.id is available
...
def get_test_status(failed_count):
    if failed_count > 0:
    ...
    for build in builds:
        for i in range(config["test_runs_per_build"]):
            ...
            test_run = TestRun(
                ...
                status=get_test_status(failed_count),  # Using Enum value
...
def generate_scenarios(db: Session, test_run: TestRun, features: List[Feature], config: Dict[str, Any]) -> None:
    ...
    for i in range(config["scenarios_per_test_run"]):
        ...
        scenario = Scenario(
            ...
            status=status,  # Using Enum value
...
def generate_report_templates(db: Session, count: int) -> List[ReportTemplate]:
    ...
    try:
        for i in range(count):
            # Use enum values directly, not string literals
            report_type_values = [ReportType.TEST_SUMMARY, ReportType.BUILD_HEALTH,
                                  ReportType.FEATURE_COVERAGE, ReportType.TREND_ANALYSIS,
                                  ReportType.CUSTOM]
            format_values = [ReportFormat.HTML, ReportFormat.PDF, ReportFormat.CSV,
                             ReportFormat.JSON, ReportFormat.XML]

            report_type_value = random.choice(report_type_values)
            format_value = random.choice(format_values)

            try:
                template = ReportTemplate(
                    name=f"Template-{report_type_value.name}-{i}",
                    description=f"Report template for {report_type_value.name} reports",
                    report_type=report_type_value,  # Use enum value directly
                    format=format_value,  # Use enum value directly
                    template_data={
                ...
                db.add(template)
                db.flush()  # Flush to get assigned IDs
                templates.append(template)
            except Exception as e:
                logger.error(f"Error creating template {i}: {e}")
                continue

        db.commit()
        logger.info(f"Created {len(templates)} report templates")
    except Exception as e:
        logger.error(f"Failed to create report templates: {e}")
        db.rollback()
...
def generate_report_schedules(db: Session, templates: List[ReportTemplate], count: int) -> None:
    ...
    try:
        ...
        if not templates:
            logger.warning("No templates available for report schedules")
            return

        for i in range(min(count, len(templates))):
            template = templates[i]

            # Debug to check template id
            logger.info(f"Creating schedule for template ID: {template.id}")
        ...
        logger.error(f"Failed to create report schedules: {e}")
        db.rollback()

def random_status() -> TestStatus:
...
def generate_reports(db: Session, templates: List[ReportTemplate], count: int) -> None:
    ...
    try:
        if not templates:
            logger.warning("No templates available for report generation")
            return

        for i in range(count):
            template = random.choice(templates)
            status_values = [ReportStatus.PENDING, ReportStatus.RUNNING,
                           ReportStatus.COMPLETED, ReportStatus.FAILED]
            status_value = random.choice(status_values)

            generated_at = datetime.datetime.now() - datetime.timedelta(
                days=random.randint(0, 30)) if status_value != ReportStatus.PENDING else None

            # Create report using ORM instead of raw SQL
            report = Report(
                template_id=template.id,
                name=f"Report-{template.report_type.name}-{i}",
                description=f"Generated report using {template.name}",
                status=status_value,
                format=template.format,
                generated_at=generated_at,
                file_path=f"/reports/{template.name}_{i}.{template.format.name.lower()}"
                    if status_value == ReportStatus.COMPLETED else None,
                file_size=random.randint(10000, 1000000)
                    if status_value == ReportStatus.COMPLETED else None,
                error_message="Failed to generate report"
                    if status_value == ReportStatus.FAILED else None,
...
def parse_args():
    ...
    parser.add_argument("--db-name", type=str, default="friday",
                        help="Database name (default: friday)")
